...
page. Five things prevent that:

1. The navigation paints with `'Open Sans Nav'`: a subset of the *same* Open
   Sans file it settles in, inlined as a data URI in `vendor/fonts/fonts.css`
   (generated by `tools/fetch-vendor-assets.mjs` from the labels in
   `_includes/header.njk`). Because it is inline there is no fetch to race, so
   the first paint already has the final metrics and the row cannot move —
   whichever fonts the machine has. This is the part that holds on Linux: the
   runner's rendering of a *local* face is not metric-stable (its CI movements
   imply per-label corrections from -1.7% to +4.1%, so no single `size-adjust`
   keeps the row still there). Nothing needs preloading or re-tuning for it, and
   the guard fails if the face is dropped, un-inlined, or stops covering a label.
2. Behind it, `styles.css` declares metric-matched fallback faces, one
   `@font-face` per system font, each with its own `size-adjust` and Open Sans'
   vertical metrics, listed straight after `'Open Sans Nav'` for the navigation
   and after `'Open Sans'` in `--font-ui` for everything else. Each face
   ...
   - `'Open Sans Fallback Liberation'` (108.8%) covers Linux. It ships with
     `fonts-liberation`, which `npx playwright install --with-deps` installs on
     the CI runner, so the Linux fallback is a known face instead of whatever
     wide generic sans the distribution ships (measured on CI before the faces
     were split: ~112% of Open Sans' advance widths, an 85px drift). The 108.8%
     is derived from the runner's own reported movements — they imply a 0.9832
     scale against the local measurement, and `107 / 0.9832` = 108.8% — **not**
     copied from Arial. Tuning the real Liberation Sans file here still puts the
     optimum at 107%; the runner renders it ~1.7% narrower, with ±2.3px of
     spread across labels.
3. The desktop wordmark box is pinned (`width: 10em` at ≥1280px). The logo sits
   ...
   the CI report).
4. `_includes/head.njk` preloads the Open Sans and Playfair Display files that
   the header needs, so the real metrics are normally there for the first paint.
5. `tests/navigation-hit-target.spec.js` guards all of it: it fails if any
   ...
   a calibrated fallback family to resolve on the running platform, asserts that
   the wordmark box does not change width when a narrow serif stands in for
   Playfair Display, and checks that the inlined navigation subset is present,
   reached before the platform faces, and covers every character of every label.
...
| A navigation click occasionally opens the wrong page or does nothing | the menu moved under the cursor while the web font loaded: rebuild and run `npx playwright test tests/navigation-hit-target.spec.js`; the inlined `'Open Sans Nav'` subset (plus the preload and the tuned local faces) keeps the items still |
